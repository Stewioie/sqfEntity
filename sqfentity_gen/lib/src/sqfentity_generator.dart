...
class SqfEntityGenerator extends GeneratorForAnnotation<SqfEntityBuilder> {
  ...
  String generateForAnnotatedElement(Element2 element, ConstantReader annotation, BuildStep buildStep) {
    ...
    final instanceName = element.toString().replaceAll('SqfEntityModel', '').replaceAll('*', '').trim();
    print('SQFENTITY GENERATOR STARTED... instance Of:$instanceName');
    final builder = SqfEntityModelBuilder(model, instanceName);
    print('SQFENTITY GENERATOR: builder initialized (${builder.instancename})...');
    ...
    final modelStr = MyStringBuffer()
          ...
          ..printToDebug('${dbModel.modelName} converted to SqfEntityModelBase successfully')
          ..writeln(SqfEntityConverter(dbModel).createEntites())
          ..printToDebug('${dbModel.modelName} converted to Entities successfully')
          ..writeln(SqfEntityConverter(dbModel).createControllers())
          ..printToDebug('${dbModel.modelName} converted to Controller successfully')
        //..writeln('*/') //  write output as commented to see what is wrong
        ;
